...
def extract_event_info(text: str, default_topic_id: int) -> dict:
    prompt = (
        ...
        "Дополнения: если просто 4 цифры например 'встал 1122' то распознавай это как время 11:22, могут быть сообщения в которых указаны оба события например 'встал 1150 слет 1155' нужно распознать это как два события started 11:50 stopped 11:55"
        "нужно вернуть только один JSON для номер, если номера нет но есть время входа и выхода то нужно вернуть время для пустого номер started true и stopped true"
